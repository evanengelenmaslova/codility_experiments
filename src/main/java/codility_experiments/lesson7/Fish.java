package codility_experiments.lesson7;
...
public class Fish {
	 ...
	 * 
	 * @param A
	 *            non empty array of N integers, N is an integer within the
	 *            range [1..100,000]. All elements are distinct.
	 * @param B
	 *            non empty array of N integers, N is an integer within the
	 *            range [1..100,000]. elements are either 0 or 1.
	 * @return
	 */
	public static int solution(int[] A, int[] B) {
		Deque<RiverFish> stack = new ArrayDeque<>();
		for (int i = 0; i < B.length; i++) {
			if (stack.isEmpty()) {// no fish in the stack yet to meet
				stack.push(new RiverFish(B[i], A[i]));
			} else {
				//there are some fish potentially to meet, check if meet condition is satisfied
				// then keep eating fish until meat condition no longer satisfied
				RiverFish newFish = new RiverFish(B[i], A[i]);
				while (!stack.isEmpty()) {
					RiverFish currentFish = stack.peek();
					if (currentFish.getDirection() == 1 && newFish.direction == 0) { // fish
																						// met
						if (currentFish.getSize() < newFish.getSize()) {
							stack.pop(); // new fish ate current fish'
							if (stack.isEmpty()) { // if all fish in stack are
													// eaten by the new fish,
													// add the new fish
								stack.push(newFish);
								break;
							}
						} else {
							break; // current fish ate new fish
						}
					} else { // fish did not meet
						stack.push(newFish);
						break;
					}
	...
	public static class RiverFish {
		int direction;
		int size;

		public RiverFish(int direction, int size) {
			super();
		...
		public boolean isBigger(RiverFish other) {
