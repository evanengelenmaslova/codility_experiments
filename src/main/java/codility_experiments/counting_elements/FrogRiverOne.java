...
public class FrogRiverOne {
	 ...
	 * Assumptions:
	 * 
	 * N and X are integers within the range [1..100,000]; each element of array
	 * A is an integer within the range [1..X].
	 * 
	 * @param X
	 *            integers within the range [1..100,000]
	 * @param A
	...
	public static int solution(int X, int[] A) {
		...
		int[] pathFilled = new int[X];
		int position = 0;
		for (int i = 0; i < A.length; i++) {
			int leafPos = A[i];
			if (leafPos <= X) {
				if (pathFilled[leafPos - 1] == 0) {
					pathFilled[leafPos - 1] = 1;
					position += 1;
					if (position == X) {
						return i;
					}
